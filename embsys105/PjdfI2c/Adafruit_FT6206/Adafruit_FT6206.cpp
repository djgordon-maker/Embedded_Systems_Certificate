...
//  pjdfErr = Ioctl(hTs, PJDF_CTRL_I2C_SET_DEVICE_ADDRESS, (uint8_t*)FT6206_ADDR, (uint32_t*)1);
...
void Adafruit_FT6206::readData(uint16_t *x, uint16_t *y) {
  ...
  uint8_t i2cdat[16];
  Write(hTs, 0, (uint32_t*) 1);
  Read(hTs, i2cdat, (uint32_t*) 16);
  /*
...
uint8_t Adafruit_FT6206::readRegister8(uint8_t reg) {
  ...
  Write(hTs, &reg, (uint32_t*)1);
  Read(hTs, &x, (uint32_t*)1);
  /*
    I2C_start(I2C1, FT6206_ADDR<<1, I2C_Direction_Transmitter); 
